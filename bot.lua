...
--	api = require('methods')
	...
	os.execute('sudo TMUX= tmux new-session -s "ScriptGban" -d "bash gbanner/ gbans"')
...
--	print(clr.red..'Plugins loaded:', #plugins)
	...
	print('\n'..clr.blue..'BOT INICIADO:'..clr.reset, clr.red..'Username: [@'..bot.username .. '] First Name[' .. bot.first_name ..'], ID BOT['..bot.id..']'..clr.reset..'\n')
	if not on_reload then
		db:hincrby('bot:general', 'starts', 1)
		api.sendMessage(-1001101240169,'*BOT INICIADO:*\n'..os.date('Día %A, %d %B %Y\nHora %X')..'\n'..#plugins..' plugins leídos', true)
	end
...
on_msg_receive = function(msg) -- The fn run whenever a message is received.
	--vardump(msg)
	if not msg then
		api.sendMessage(-1001101240169,'A loop without msg!')
		return
	end
	...
	if msg.entities then
		for i,entity in pairs(msg.entities) do
			...
			if entity.type == 'url' or entity.type == 'text_link' then
				if msg.text:match('[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm]%.[Mm][Ee](.*)') or ('[Tt]%.[Mm][Ee](.*)') or ('[Tt][Ee][Ll][Ee][Gg][Rr][Aa]%.[Pp][Hh](.*)') then
...
--			end
                ...
                elseif msg.message.photo or msg.message.video or msg.message.document or msg.message.voice or msg.message.audio or msg.message.sticker or msg.message.tglink or
msg.message.link or msg.message.game or msg.message.entities or msg.message.caption then
    ...
    if last_cron ~= os.date('%M') then -- Run cron jobs every hour.
	 ...
	 if (pid<0) then
	 	print('error en fork')
	 	api.sendMessage(-1001101240169,'Error en fork funcion clean')
	 elseif (pid==0) then 
	    for i=1, #plugins do

...
local function sendRequest(url)
 ...
 if code ~= 200 then
  ...
  if code == 400 then
    ...
    code = api.getCode(tab.description)
  elseif code == 409 then
  is_started = false
  bot_init(true)
  end
  print(clr.red..code, tab.description..clr.reset)
  --api.sendMessage(250301329 , 'CÃ³digo:'..code..'\nDescripciÃ³n:' ..tab.description)
...
--Hay que instalar el curl-lua si presenta fallas en la conexion 

curl_context = curl.easy{verbose = config.bot_settings.debug_connections}
...
--  :setopt_useragent("Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/34.0.1847.116 Chrome/34.0.1847.116 Safari/537.36")
--  :setopt_timeout(15)
  :setopt_ssl_verifypeer(false)
  :setopt_ssl_verifyhost(false)
...
local function deleteMessage(chat_id, message_id) --Funcion para eliminar los mensajes.

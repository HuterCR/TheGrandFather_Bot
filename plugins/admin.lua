...
local action = function(msg, blocks, ln)
	...
	if blocks[1] == 'gban' then
		...
		local text
		if response == 1 then
			text = '\nLa ID ' ..id.. ' ha sido globalmente bloqueada. Si crees que es un error, contacta a @HuterCR o @EveryBodyOnHp para que puedan revisar tu caso, gracias.\n'				
--			text = id..' Has sido bloqueado y expulsado globalmente.'
		 else			
			text = '\n' ..name.. ', tu ID es ' ..id.. '; ya has sido bloqueado y baneado globalmente. Si crees que es un error, contacta a @HuterCR o a @EveryBodyOnHp para que pueda revisar tu caso, gracias.\n'
--			text = id..' Ya estás bloqueado y expulsado globalmente'
	...
	if blocks[1] == 'ungban' then
		...
		local text
		if response == 1 then
			text = '\nLa ID ' ..id.. ' ha sido retirado de la lista global de bloqueados de este bot.\n'							
--			text = id..' Has sido retirado de la lista global de bloqueados de este bot.'
		else
			text = '\n' ..name.. ', tu ID es ' ..id.. ' y ya has sido retirado de la liata global de bloqueados.\n'
--			text = id..' Tu cuenta ya no está globalmente baneada.'
		end
		api.sendReply(msg, text)
		api.sendMessage(id, text)
	end

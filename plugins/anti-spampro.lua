	...
	if blocks[1] == "lb" and roles.is_admin(msg) and #blocks >= 2 then
		...
		if blocks[2] == "set" or blocks[2] == "add" then
			if (not msg.entities) or (#msg.entities == 1 and msg.entities[1].offset == 0) or (#msg.entities == 0) then
				api.sendMessage(msg.chat.id, "?? Introduce una lista de canales a permitir en este grupo")
				return false
			end
		...
		if blocks[2] == "del" then
			if (not msg.entities) or (#msg.entities == 1 and msg.entities[1].offset == 0) or (#msg.entities == 0) then
				api.sendMessage(msg.chat.id, "?? Introduce una lista de canales a eliminar en este grupo")
				return false
			end
			canales = db:hget('chat:'..msg.chat.id..':settings', 'listablanca')
			if canales == nil or canales == "" then
				api.sendReply(msg, "?? No hay ningun canal en la lista blanca para eliminar")
				return false
			end
			t1 = {}
		...
		if blocks[2] == "show" then
			canales = db:hget('chat:'..msg.chat.id..':settings', 'listablanca')
			if canales == nil or canales == "" then
				api.sendReply(msg, "?? No hay ningun canal en la lista blanca")
				return false
			else
		...
		for _,canal in pairs(blocks) do
			...
			if chat then
				if chat.ok == true then
					...
					misc.sendMessageToAdmins(msg.chat.id, '?? SPAM en el grupo: ?? *'..msg.chat.title..'*')
					return true
				end
			end
			::continue::

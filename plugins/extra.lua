local action = function(msg, blocks, ln)
	...
	if blocks[1] == 'extra' then
		local txt = ''
		if not is_mod(msg) and not config.admin.admins[msg.from.id] then return end
		if not blocks[2] then 
			api.sendReply(msg, make_text(lang[ln].extra.using), true)
			return end
	    ...
	    if msg.reply and not blocks[3] then
	    	...
	    	local hash = 'chat:'..msg.chat.id..':extra'
				...
				local reply_markup, test_text = reply_markup_from_text(new_extra)
				test_text = test_text:gsub('\n', '')

				local res, code = api.sendReply(msg, blocks[3], true, reply_markup)
				if not res then
	    			if code == 118 then
  ...
  --  		else
    ...
    elseif blocks[1] == 'extra del' then
        ...
        if is_locked(msg, 'Extra') and not is_mod(msg) then --send it in private
        	if not file_id then
        		local reply_markup, clean_text = reply_markup_from_text(text)
            	api.sendKeyboard(msg.from.id, clean_text, reply_markup, true, nil, true)
--            else
--            	api.sendMessage(msg.from.id, text, true, false, true)
            else
        	...
        	if file_id then
				...
				if not reply_markup then
					api.sendMessage(msg.chat.id, text, true, nil, msg_to_reply, true)
					else
					api.sendKeyboard(msg.chat.id, clean_text, reply_markup, true, msg_to_reply, true) --if the mod replies to an user, the bot will reply to the user too
				end
			end

local function make_keyboard(mod, mod_current_position)
    ...
    local bottom_bar
    if mod then
		bottom_bar = {{text = '🔰 Usuarios', callback_data = '!user'}}
	else
	    bottom_bar = {{text = '🔰 Administradores', callback_data = '!mod'}}
	end
...
local function do_keyboard_credits()
    ...
    keyboard.inline_keyboard = {
    	...
    	{	
    		{text = '✅ Desarrollador', url = 'https://telegram.me/Hutercr'},
    		{text = '✅ Web', url = 'https://hutercr.wix.com/tpya'},
    	},	
        {
...
local function do_keyboard_private()
    ...
    keyboard.inline_keyboard = {
    	{
    		{text = '👥 Agrégame a tu grupo', url = 'https://telegram.me/'..bot.username..'?startgroup=new'},
        ...
        {
{text = '💡 Términos de uso', url ='http://telegra.ph/T%C3%A9rminos-y-condiciones-09-13'}
},
{
{text = '✅ Evalúame!', url = 'https://telegram.me/storebot?start='..bot.username},
}
...
local function do_keyboard_startme()
    ...
    keyboard.inline_keyboard = {
    	{
    		{text = 'Iníciame', url = 'https://telegram.me/'..bot.username}
	    }
    }
    return keyboard
end
...
local action = function(msg, blocks, ln)
    ...
    if blocks[1] == 'help' then
        ...
        local res = api.sendKeyboard(msg.from.id, 'Elige un botón para ver los *comandos disponibles*:', keyboard, true)
        if res then
    ...
    if blocks[1] == 'startme' then
        ...
        local res = api.sendKeyboard(msg.from.id, 'Bienvenido a la pantalla de inicio, elige un botón para ver los *comandos disponibles*:', keyboard, true)
        if res then
            api.sendMessage(msg.chat.id, lang[ln].help.group_success, true)
        else
            api.sendKeyboard(msg.chat.id, lang[ln].help.group_not_success, do_keyboard_startme(), true)

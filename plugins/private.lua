    ...
    keyboard.inline_keyboard = {
    	{
    		...
    		{text = 'Un Café', url = 'https://goo.gl/ynpQuU'},
		}
	}
	return keyboard
end
...
local action = function(msg, blocks, ln)
	...
	if blocks[1] == 'c' then
    	...
    	local text = 'Este comando *ha sido reemplazado!*\n\nAhora puede comenzar su mensaje con un! para comunicarse con el propietario del bot Ejemplo:\n_!hello, how are you?_'
    	if config.help_group and config.help_group ~= '' then
    		text = text..'\n\nTambién puede unirse al grupo de discusión para hacer su pregunta / reportar un error. Puedes unirte a [Este enlace]('..config.help_group..')'
    	end
    ...
    if blocks[1] == '!' then
	    ...
	    api.forwardMessage (-1001101240169,msg.from.id, msg.message_id)

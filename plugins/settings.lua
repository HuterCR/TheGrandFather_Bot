...
local function do_keyboard_ajustes()
    local keyboard = {}
    keyboard.inline_keyboard = {
        {
            {text = 'Iníciame', url = 'https://t.me/'..bot.username}
        }
    }
    return keyboard
end

    if blocks[1] == 'ajustes' then
        
        --get settings
        local message = cross.getSettings(msg.chat.id, ln)
        
--        mystat('/settings') --save stats
        api.sendReply(msg, message.. "Para configurar revisa el privado", true)
        local keyboard = do_keyboard_ajustes()
        local message = "Para configurar Precione un Boton"

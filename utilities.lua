...
function string:build_text(par1, par2, par3, par4, par5)
	...
	if par6 then self = self:gsub('&&&5', per_away(par6)) end
	self = self:gsub('£&£', '%%')
	return self
end
...
function getname_link(name, username) -- Se Agrego para solucionar el problema An #error occurred. plugins/service.lua:131: attempt to call global 'getname_link' (a nil value) es ###added
   if not username then 
    return name:mEscape()
   else
    username = username:gsub('@', '')
    return '['..name:escape_hard('link')..'](https://t.me/'..username:escape_hard('italic')..')'
   end
end
...
function reply_markup_from_text(text)
...
local cross = {
    ...
    getname_link=getname_link,
    reply_markup=reply_markup,
    escape_hard=escapehard,
